...
class SensorModel:
    ...
    def raycast(self, rob_x,rob_y,rob_theta,laser_angle,step):
        ...
        x_final = x_start
        y_final = y_start
        while 0 <= x_final < self.map.shape[0] and 0 <= y_final < self.map.shape[1] and abs(self.map[y_final, x_final]) < self._min_probability:
    ...
    def beam_range_finder_model(self, z_t1_arr, x_t1):
        ...
        rob_y = x_t1[1]
        rob_theta = x_t1[2]
        for i in range (-90,90, self._subsampling ):
